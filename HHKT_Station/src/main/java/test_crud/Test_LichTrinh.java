...
public class Test_LichTrinh {
    public static void main(String[] args) {
        ...
        LichTrinh_DAO lichTrinh_dao = new LichTrinh_DAO(em);
        CT_LichTrinh_DAO ctLichTrinh_dao = new CT_LichTrinh_DAO(em);
        try {
            ChuyenTau ct = new ChuyenTau("SE2", new LoaiTau("SE"));
            new ChuyenTau_DAO(em).create(ct);
            new Toa_DAO(em).create(new Toa("SE2T1", 1, new ChuyenTau("SE2"), new LoaiToa("TVIP")));
            new ChoNgoi_DAO(em).create(new ChoNgoi("SE2T1CN1", new Toa("SE2T1"), 1, 1, 1));
            new ChoNgoi_DAO(em).create(new ChoNgoi("SE2T1CN2", new Toa("SE2T1"), 2, 1, 1));
